...
PLOT_TYPES = ['generate-samples', 'data-samples', 'reconstruct', "traversals",
              'reconstruct-traverse', "gif-traversals", "gaussians", "all"]
...
def parse_arguments(args_to_parse):
    ...
    parser.add_argument('-d', '--dataset', help="Path to training data.",
                          default=dataset_config['test_dataset'])
...
def main(args):
    ...
    samples = get_samples(dataset, num_samples, idcs=args.idcs, random_ids=random_samples, train_fld=args.train_folder)
    ...
    for plot_type in args.plots:
        ...
        elif plot_type == 'gaussians':
            viz.plot_gaussians(data=samples[0:1, ...])
