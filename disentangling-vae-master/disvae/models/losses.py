...
class FactorKLoss(BaseLoss):
    ...
    def call_optimize(self, data, model, optimizer, storer, normals=None, labels=None, objective=None, first_iteration=False):
        ...
        if first_iteration:
            self.global_iteration += 1
            # Using precomputed values, avoiding unnecessary computation during training
        ...
        rec_loss = _reconstruction_loss(objective1, recon_batch,
                                            storer=storer,
                                            distribution=self.rec_dist,
                                            labels=labels,
...
class BtcvaeLoss(BaseLoss):
    ...
    def __init__(self, n_data, alpha=1., beta=6., gamma=1., is_mss=False, **kwargs):
        super().__init__(**kwargs)
        self.n_data = n_data
        self.beta = beta
        self.alpha = alpha
...
def _reconstruction_loss(data, recon_data, distribution="bernoulli", storer=None, labels=None, vgg=None):
    """
...
def _kl_normal_loss(mean, logvar, storer=None, kl_func=0):
    ...
    latent_kl = 0.5 * (-1 - logvar + mean.pow(2) + logvar.exp()).mean(dim=0) # kl formula as in the paper
    if kl_func == 0:
