...
def load_model(directory, is_gpu=True, filename=MODEL_FILENAME, use_normals=True, skip=1):
    ...
    path_to_model = os.path.join(directory, filename)
    model = _get_model(model_type, img_size, latent_dim, device, path_to_model, use_normals, skip)
    return model
...
def _get_model(model_type, img_size, latent_dim, device, path_to_model, use_normals=False, skip=0):
    """ Load a single model.
    ...
    model = init_specific_model(model_type, img_size, latent_dim, use_normals=use_normals, skip=skip).to(device)
